...
class Point_SLAM():
    ...
    def __init__(self, cfg, args, share_npc=True, share_decoders=True, time_string=None):
        ...
        if cfg['mapping']['save_rendered_image']:
            ...
            os.makedirs(f'{self.output}/segments', exist_ok=True)
            os.makedirs(f'{self.output}/refinement', exist_ok=True)
            os.makedirs(f'{self.output}/orb', exist_ok=True)
            os.makedirs(f'{self.output}/segments_error', exist_ok=True)
        ...
        # , color="/home/esandstroem/scratch-second/esandstroem/point-slam-loop/pretrained/01999.tar")
        ...
        self.camera_before_correction = torch.zeros((1, 4, 4))
        self.camera_before_correction.share_memory_()
        self.use_cbc = torch.tensor([False])
        self.use_cbc.share_memory_()
        ...
        if share_npc:
            ...
            manager = BaseManager()
            manager.start()
            self.npc = manager.NeuralPointCloud(cfg, self, args)
        else:
            self.npc = NeuralPointCloud(cfg, self, args)
    ...
    def load_pretrain(self, cfg, color=None):
        """
        ...
        if color:
            color_dict = {}
            ckpt = torch.load(color)
            decoders = ckpt["decoder_state_dict"]
            for k, v in decoders.items():
                if "color" in k:
                    color_dict[k[14:]] = v

            self.shared_decoders.color_decoder.load_state_dict(
                color_dict, strict=False)
